...
Public Class CutsceneManager
    ...
    Public Shared Function RaycastEntity(ByVal screenCoord As Vector2, ByVal camPos As Vector3, ByVal camRot As Vector3) As Entity
        ...
        vector2.Normalize()
        Dim result As RaycastResult = World.Raycast((vector + CType((vector2 * 1.0!), Vector3)), (vector + CType((vector2 * 100.0!), Vector3)), (IntersectOptions.Vegetation Or (IntersectOptions.Objects Or (IntersectOptions.Peds1 Or (IntersectOptions.Mission_Entities Or IntersectOptions.Map)))), character)
    ...
    Public Shared Function RaycastEverything(ByVal screenCoord As Vector2) As Vector3
        ...
        vector5.Normalize()
        Dim result As RaycastResult = World.Raycast((vector4 + CType((vector5 * 1.0!), Vector3)), (vector4 + CType((vector5 * 100.0!), Vector3)), (IntersectOptions.Vegetation Or (IntersectOptions.Objects Or (IntersectOptions.Peds1 Or (IntersectOptions.Mission_Entities Or IntersectOptions.Map)))), character)
        If result.DitHitAnything Then
            Return result.HitCoords
        End If
        Return (position + CType((vector5 * 100.0!), Vector3))
    End Function
    ...
    Public Shared Function RaycastEverything(ByVal screenCoord As Vector2, ByVal camPos As Vector3, ByVal camRot As Vector3, ByVal toIgnore As Entity) As Vector3
        ...
        Dim vector2 As Vector3 = (CutsceneManager.ScreenRelToWorld(camPos, camRot, screenCoord) - vector)
        vector2.Normalize()
        Dim result As RaycastResult = World.Raycast((vector + CType((vector2 * 1.0!), Vector3)), (vector + CType((vector2 * 100.0!), Vector3)), (IntersectOptions.Vegetation Or (IntersectOptions.Objects Or (IntersectOptions.Peds1 Or (IntersectOptions.Mission_Entities Or IntersectOptions.Map)))), ignoreEntity)
        If result.DitHitAnything Then
            Return result.HitCoords
        End If
        Return (camPos + CType((vector2 * 100.0!), Vector3))
    End Function
    ...
    Public Shared Function ScreenRelToWorld(ByVal camPos As Vector3, ByVal camRot As Vector3, ByVal coord As Vector2) As Vector3
        ...
        Dim vector6 As Vector3 = CType(((vector1 * CSng(Math.Cos(d))) - (vector5 * CSng(Math.Sin(d)))), Vector3)
        Dim vector7 As Vector3 = CType(((vector1 * CSng(Math.Sin(d))) + (vector5 * CSng(Math.Cos(d)))), Vector3)
        If Not CutsceneManager.WorldToScreenRel((((camPos + CType((vector * 10.0!), Vector3)) + vector6) + vector7), vector8) Then
            Return (camPos + CType((vector * 10.0!), Vector3))
        End If
        If Not CutsceneManager.WorldToScreenRel((camPos + CType((vector * 10.0!), Vector3)), vector9) Then
            Return (camPos + CType((vector * 10.0!), Vector3))
        End If
        ...
        Return CType((((camPos + (vector * 10.0!)) + (vector6 * num2)) + (vector7 * num3)), Vector3)
    End Function
