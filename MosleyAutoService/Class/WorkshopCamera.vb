...
Public Class WorkshopCamera
            ...
            Case CameraPosition.Car
                If True Then
                    ...
                    ElseIf _internalCameraPosition = CameraPosition.Interior Then
                        RepositionFor(CType(_target, Vehicle))
                    End If
                End If
                Exit Select
    ...
    Public Sub RepositionFor(lowrider As Vehicle, Optional cameraPos As Vector3 = Nothing, Optional cameraRot As Vector3 = Nothing)
        World.DestroyAllCameras()
        _mainCamera = World.CreateCamera(lowrider.Position - lowrider.ForwardVector * 5.0 + New Vector3(0, 0, 1.5), CutsceneManager.DirectionToRotation(lowrider.ForwardVector * -5.0), GameplayCamera.FieldOfView)
        _mainCamera.PointAt(lowrider)
        If Not cameraPos = Nothing Then _mainCamera.Position = cameraPos
        If Not cameraRot = Nothing Then _mainCamera.Rotation = cameraRot
        World.RenderingCamera = _mainCamera
        _target = lowrider
        _targetPos = lowrider.Position
        _cameraZoom = 5.0
    ...
    Public Function Update() As Tuple(Of Vector3, Vector3)
        If _mainCamera Is Nothing Then
            Return New Tuple(Of Vector3, Vector3)(_mainCamera.Position, _mainCamera.Rotation)
        End If
        ...
        If Game.IsControlJustPressed(0, Control.Attack) AndAlso Not _isDragging AndAlso Not IsMouseInMenu() Then
            ...
            Native.Function.Call(CType(&H8DB8CFFD58B62552UL, Hash), 4)
        ...
        If Game.IsControlJustReleased(0, Control.Attack) AndAlso _isDragging Then
            ...
            Native.Function.Call(CType(&H8DB8CFFD58B62552UL, Hash), 0)
        End If
